...
function makeHistory(current) {
  return Array.from({ length: 10 }, (_, i) =>
    i === 9
      ? current
      : Math.max(30, Math.min(100, current + (Math.random() - 0.5) * 20)),
...
function SkeletonCard() {
  return (
    ...
    >
      ...
      <div
        className="skeleton"
        style={{ height: 48, width: "60%", marginBottom: 12 }}
      />
      <div
        className="skeleton"
        style={{ height: 10, width: "80%", marginBottom: 8 }}
      />
...
function ZoneCard({ zone, onClick }) {
  ...
  const trend =
    zone.history && zone.history.length >= 2
      ? zone.history[zone.history.length - 1].v -
        zone.history[zone.history.length - 2].v
      : 0;
  ...
  async function fetchData(initial = false) {
    try {
      ...
      const enriched = data.map((z) => {
        if (!historyMap.current[z.zone])
          historyMap.current[z.zone] = makeHistory(z.stress);
        else {
          historyMap.current[z.zone] = [
            ...historyMap.current[z.zone].slice(1),
            { t: Date.now(), v: z.stress },
          ];
        }
        ...
        if (z.level === "HIGH" && (isNew || prev !== "HIGH")) {
          ...
          setAlerts((a) => [
            ...a.slice(-3),
            {
              ...
              level: z.level,
              stress: z.stress,
              message: `${z.zone} is under HIGH stress`,
            },
          ]);
          setTimeout(
            () => setAlerts((a) => a.filter((x) => x.id !== id)),
          ...
          setAlerts((a) => [
            ...a.slice(-3),
            {
              ...
              level: z.level,
              stress: z.stress,
              message: `${z.zone} recovered to ${z.level}`,
            },
          ]);
          setTimeout(
            () => setAlerts((a) => a.filter((x) => x.id !== id)),
    ...
    >
      ...
      <main style={{ padding: "40px 32px", maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 36 }}>
          ...
          >
            Real-time monitoring across {zones.length} zones · auto-refreshes
            every 3 seconds
          </p>
        </div>
        ...
        {!loading && !error && (
          <div style={{ display: "flex", gap: 12, marginBottom: 32 }}>
            {[
              ...
              {
                label: "LOW / Stable",
                value: zones.length - highCount - mediumCount,
                color: "#34d399",
              },
            ].map((s) => (
              <div
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 20 }}>
          {loading
            ...
            : zones.map((z) => (
                <ZoneCard key={z.zone} zone={z} onClick={setModal} />
              ))}
        </div>
