...
# ─── Hospital Layout ─────────────────────────────────────────────────────────
WARDS = {
    "ICU":       {"rooms": ["ICU-1", "ICU-2", "ICU-3", "ICU-4"],      "bed_capacity": 4, "base_stress": 65, "variance": 20},
    "General":   {"rooms": ["GEN-1", "GEN-2", "GEN-3", "GEN-4", "GEN-5", "GEN-6"], "bed_capacity": 6, "base_stress": 42, "variance": 18},
    "Emergency": {"rooms": ["ER-1", "ER-2", "ER-3", "ER-4"],           "bed_capacity": 4, "base_stress": 72, "variance": 22},
}
...
class StressPredictor:
    ...
    def recommend(self, score: float, ward: str, features: dict) -> str:
        ...
        if level == "HIGH":
            if deficit > 60:
                return f"⚠ {ward}: Critical understaffing — immediate reallocation needed"
            elif occupancy > 90:
...
def generate_room_data(room_id: str, ward: str, dt: Optional[datetime] = None) -> dict:
    ...
    patients_count = max(1, min(max_patients, round(expected_occ + gaussian_noise(0, 0.8))))
    ...
    nurses_on_duty  = max(0, min(6, required_nurses + nurse_jitter - (1 if time_mult > 0.70 else 0)))
    ...
    active_alerts   = max(0, min(max_active,   round(occ_rate * 4 * time_mult + gaussian_noise(0, 0.7))))
    critical_alerts = max(0, min(max_critical, round(occ_rate * 1.5 * time_mult + gaussian_noise(0, 0.5))))
    ...
    features = {
        ...
        "staff_deficit_rate":   max(0, (required_nurses - nurses_on_duty) / max(required_nurses, 1)) * 100,
...
def build_synthetic_dataset(n_snapshots: int = 100, interval_minutes: int = 15,
                             output_json: str = "dataset.json",
                             output_csv:  str = "dataset.csv") -> list:
    """
...
if __name__ == "__main__":
    ...
    if "--dataset" in sys.argv:
        ...
        print("🧠 Building synthetic training dataset (100 snapshots × 15-min intervals)...")
        ...
        print(f"HIGH stress rooms  : {sum(1 for r in data if r['stress_level'] == 'HIGH')}")
        print(f"MEDIUM stress rooms: {sum(1 for r in data if r['stress_level'] == 'MEDIUM')}")
        print(f"LOW stress rooms   : {sum(1 for r in data if r['stress_level'] == 'LOW')}")
